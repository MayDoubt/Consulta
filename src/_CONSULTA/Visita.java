...
public class Visita {
	...
	public Visita(Paciente paciente, LocalDate fecha) {
		...
		this.tarifa = generarTarifa((this.especialidad.equalsIgnoreCase(listaEspecialidades[0]) ? 0 : 1));
	...
	public String toString(Profesional[] registroEspecialistas) {
		...
		if (urgencia.equals("Si")) {
			return "Tarifa=" + String.format("%.2f", tarifa * 1.2) + " €\t Urgencia=" + urgencia + "\t IdEspecialista="
					+ idEspecialista + "\t NombreEsp="
					+ String.format("%-10s", registroEspecialistas[idEspecialista].nombre) + "\t Especialidad="
					+ especialidad + "\t IdPaciente=" + String.format("%04d", idPaciente) + "\t Fecha=" + fecha.format(shortFormat)
					+ "\t FormaPago=" + formaPago;
		} else {
			return "Tarifa=" + String.format("%.2f", tarifa) + " €\t Urgencia=" + urgencia + "\t IdEspecialista="
					+ idEspecialista + "\t NombreEsp="
					+ String.format("%-10s", registroEspecialistas[idEspecialista].nombre) + "\t Especialidad="
					+ especialidad + "\t IdPaciente=" + String.format("%04d", idPaciente) + "\t Fecha="
